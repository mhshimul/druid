...
public class IndexerOracleMetadataStorageCoordinator implements IndexerMetadataStorageCoordinator
{
  ...
  public List<DataSegment> getUsedSegmentsForIntervals(final String dataSource, final List<Interval> intervals)
      throws IOException
  {
    return connector.retryWithHandle((HandleCallback<List<DataSegment>>) handle -> {
      ...
      Set<DataSegment> segments = Sets.newHashSet(Iterables.concat(Iterables.transform(Iterables.concat(Iterables.transform(
          intervals,
          (Function<Interval, Iterable<TimelineObjectHolder<String, DataSegment>>>) timeline::lookup
      )), input -> input != null ? input.getObject().payloads() : null)));
  ...
  ) throws IOException
  {
    ...
    for (int i = 0; i < intervals.size(); i++) {
      sb.append(String.format("(%1$sstart%1$s <= ? AND %1$send%1$s >= ?)", connector.getQuoteString()));
    ...
    Query<Map<String, Object>> sql = handle.createQuery(String.format(sb.toString(), dbTables.getSegmentsTable()))
                                           .bind(0, dataSource);
  ...
  ) throws IOException
  {
    ...
    try {
      return connector.retryTransaction((handle, transactionStatus) -> {
        ...
        for (final DataSegment segment : segments) {
          if (announceHistoricalSegment(handle, segment, usedSegments.contains(segment) ? "true" : "false")) {
  ...
  ) throws IOException
  {
    ...
    final String previousSegmentIdNotNull = previousSegmentId == null ? "empty" : previousSegmentId;

    return connector.retryTransaction((handle, transactionStatus) -> {
      final List<byte[]> existingBytes = handle.createQuery(String.format(
      ...
      if (existingChunks.size() > 1) {
        ...
        for (SegmentIdentifier pending : pendings) {
          if (max == null || pending.getVersion().compareTo(max.getVersion()) > 0 || (pending.getVersion()
                                                                                             .equals(max.getVersion())
                                                                                      && pending.getShardSpec()
                                                                                                .getPartitionNum() > max
                                                                                             .getShardSpec()
                                                                                             .getPartitionNum())) {
            max = pending;
   ...
   */
  private boolean announceHistoricalSegment(final Handle handle, final DataSegment segment, final String used)
      throws IOException
  {
    try {
  ...
  {
    return !handle.createQuery(String.format("SELECT id FROM %s WHERE id = :identifier", dbTables.getSegmentsTable()))
   ...
   */
  private byte[] getDataSourceMetadataWithHandleAsBytes(final Handle handle, final String dataSource)
  {
  ...
  ) throws IOException
  {
    ...
    if (oldCommitMetadataBytesFromDb == null) {
      ...
      oldCommitMetadataSha1FromDb = BaseEncoding.base16()
                                                .encode(Hashing.sha1()
                                                               .hashBytes(oldCommitMetadataBytesFromDb)
                                                               .asBytes());
    ...
    final String newCommitMetadataSha1 = BaseEncoding.base16()
                                                     .encode(Hashing.sha1()
                                                                    .hashBytes(newCommitMetadataBytes)
                                                                    .asBytes());

    final DataSourceMetadataUpdateResult retVal;
    if (oldCommitMetadataBytesFromDb == null) {
      // SELECT -> INSERT can fail due to races; callers must be prepared to retry.
      final int numRows = handle.createStatement(String.format(
  ...
  public boolean deleteDataSourceMetadata(final String dataSource)
  {
    return connector.retryWithHandle(handle -> {
      int rows = handle.createStatement(String.format(
  ...
  private void updatePayload(final Handle handle, final DataSegment segment) throws IOException
  {
    try {
      handle.createStatement(String.format(
          ...
          dbTables.getSegmentsTable()
      )).bind("id", segment.getIdentifier()).bind("payload", jsonMapper.writeValueAsBytes(segment)).execute();
    }
    catch (IOException e) {
  ...
  {
    List<DataSegment> matchingSegments = connector.getDBI()
                                                  .withHandle(handle -> handle.createQuery(String.format(
                                                      "SELECT payload FROM %1$s WHERE dataSource = :dataSource and %2$sstart%2$s >= :start and %2$send%2$s <= :end and used = :used",
                                                      dbTables.getSegmentsTable(),
                                                      connector.getQuoteString()
                                                  ))
                                                                              .setFetchSize(100)
                                                                              ...
                                                                              .fold(
                                                                                  ...
                                                                                  (Folder3<List<DataSegment>, byte[]>) (accumulator, payload, foldController, statementContext) -> {
                                                                                    try {
                                                                                      accumulator.add(jsonMapper.readValue(
                                                                                          payload,
                                                                                          DataSegment.class
                                                                                      ));
                                                                                      return accumulator;
