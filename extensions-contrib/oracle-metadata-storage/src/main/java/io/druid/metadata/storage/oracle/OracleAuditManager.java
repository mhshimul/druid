...
@ManageLifecycle
public class OracleAuditManager implements AuditManager
{
  ...
  public void doAudit(AuditEntry auditEntry, Handle handle) throws IOException
  {
    ...
    handle.createStatement(String.format(
        "INSERT INTO %s ( id, audit_key, type, author, \"comment\", created_date, payload) VALUES (druid_audit_seq.nextval, :audit_key, :type, :author, :comment, :created_date, :payload)",
        getAuditTable()
    ))
  ...
  public List<AuditEntry> fetchAuditHistory(final String key, final String type, Interval interval)
  {
    ...
    return dbi.withHandle(handle -> handle.createQuery(String.format(
      ...
      throws IllegalArgumentException
  {
    ...
    return dbi.withHandle(handle -> {
      ...
      return query.bind("type", type).setMaxRows(theLimit).map((index, r, ctx) -> {
        try {
          return jsonMapper.readValue(r.getBytes("payload"), AuditEntry.class);
        }
        catch (IOException e) {
