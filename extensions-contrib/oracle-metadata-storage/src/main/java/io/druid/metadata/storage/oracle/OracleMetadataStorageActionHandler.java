    ...
    implements MetadataStorageActionHandler<EntryType, StatusType, LogType, LockType>
{
    ...
    try {
      connector.retryWithHandle((HandleCallback<Void>) handle -> {
        ...
        return null;
          ...
          final boolean isStatementException = e instanceof StatementException || (e instanceof CallbackFailedException
    ...
    return connector.retryWithHandle(
        handle -> {
          byte[] res = handle.createQuery(
              String.format(
                  "SELECT payload FROM %s WHERE id = :id", entryTable)
          )
                             .bind("id", entryId)
  ...
  public Optional<StatusType> getStatus(final String entryId)
  {
    return connector.retryWithHandle(
        handle -> {
          byte[] res = handle.createQuery(String.format("SELECT status_payload FROM %s WHERE id = :id", entryTable))
                             .bind("id", entryId)
                             .map(ByteArrayMapper.FIRST)
                             .first();
  ...
  {
    return connector.retryWithHandle(handle -> handle.createQuery(String.format(
        "SELECT id, payload, status_payload FROM %s WHERE active = 'true' ORDER BY created_date",
        entryTable
    )).map((index, r, ctx) -> {
      try {
        return Pair.of(
            jsonMapper.<EntryType>readValue(r.getBytes("payload"), entryType),
            jsonMapper.<StatusType>readValue(r.getBytes("status_payload"), statusType)
        );
      }
      catch (IOException e) {
  ...
  public List<StatusType> getInactiveStatusesSince(final DateTime timestamp)
  {
    return connector.retryWithHandle(handle -> handle.createQuery(String.format(
        ...
        entryTable
    )).bind("start", timestamp.toString()).map((ResultSetMapper<StatusType>) (index, r, ctx) -> {
      try {
  ...
  {
    return connector.retryWithHandle(handle -> handle.createStatement(String.format(
        "INSERT INTO %1$s (id, %2$s_id, lock_payload) VALUES (%1$s_seq.nextval, :entryId, :payload)",
        lockTable,
        entryTypeName
    )).bind("entryId", "druid_" + entryId).bind("payload", jsonMapper.writeValueAsBytes(lock)).execute() == 1);
  }
  ...
  public boolean addLog(final String entryId, final LogType log)
  {
    return connector.retryWithHandle(handle -> handle.createStatement(String.format(
        ...
        logTable,
        entryTypeName
    )).bind("entryId", entryId).bind("payload", jsonMapper.writeValueAsBytes(log)).execute() == 1);
  }
  ...
  public List<LogType> getLogs(final String entryId)
  {
    return connector.retryWithHandle(handle -> handle.createQuery(String.format(
        ...
        entryTypeName
                                                     ...
                                                     .fold(
                                                         Lists.newLinkedList(),
                                                         (Folder3<List<LogType>, byte[]>) (list, bytes, control, ctx) -> {
                                                           try {
    ...
    return connector.retryWithHandle(handle -> handle.createQuery(String.format(
        ...
        entryTypeName
                                                     ...
                                                     .map((index, r, ctx) -> {
                                                       try {
                                                         ...
                                                         log.makeAlert(e, "Failed to deserialize " + lockType.getType())
                                                            ...
                                                            .addData(
                                                                "lockPayload",
                                                                StringUtils.fromUtf8(r.getBytes("lock_payload"))
                                                            )
                                                            .emit();
                                                         throw new SQLException(e);
                                                       }
                                                     ...
                                                     .fold(
                                                         ...
                                                         (Folder3<Map<Long, LockType>, Pair<Long, LockType>>) (accumulator, lock, control, ctx) -> {
