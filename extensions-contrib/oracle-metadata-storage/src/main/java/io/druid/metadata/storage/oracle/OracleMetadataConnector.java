...
public class OracleMetadataConnector extends SQLMetadataConnector
{
  ...
  public boolean tableExists(Handle handle, String tableName)
  {
    ...
    boolean exists = !handle.createQuery("select * from user_tables where table_name = :tableName")
  ...
  private void createAuditTable(final String tableName)
  {
    createTable(
        ...
        ImmutableList.of(
            ...
            String.format(
                "CREATE SEQUENCE %1$s_seq MINVALUE 1 START WITH 1 INCREMENT BY 1 NOCACHE",
                tableName
            ),
            String.format(
                "CREATE INDEX idx_%1$s_key_time ON %1$s(audit_key, created_date)",
                tableName
            ),
            String.format(
                "CREATE INDEX idx_%1$s_type_time ON %1$s(type, created_date)",
                tableName
            ),
  ...
  public void createLogTable(final String tableName, final String entryTypeName)
  {
    createTable(
        ...
        ImmutableList.of(
            ...
            String.format("CREATE INDEX idx_%1$s_%2$s_id ON %1$s(%2$s_id)",
                          tableName,
                          entryTypeName
            )
        )
